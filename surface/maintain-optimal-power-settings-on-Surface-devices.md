...
Per garantire che i dispositivi Surface nell'organizzazione trarranno vantaggio dalle funzionalità di ottimizzazione dell'alimentazione surface:
...
- Evita di creare profili di alimentazione personalizzati o di modificare le impostazioni di risparmio energia avanzate non visibili nell'interfaccia utente predefinita (**SystemPower** >  **& sospensione**).
- Se devi gestire il profilo di alimentazione dei dispositivi nella rete (ad esempio nelle organizzazioni altamente gestite), usa lo strumento di comando powercfg per esportare la combinazione per il risparmio di energia dall'immagine di fabbrica del dispositivo Surface e quindi importarla nel pacchetto di provisioning per i dispositivi Surface. 
    ...
    >[!NOTE]
    >Puoi esportare una combinazione per il risparmio di energia solo nello stesso tipo di dispositivo Surface.  Ad esempio, non è possibile esportare una combinazione per il Surface Laptop e importarla in Surface Pro.  Per ulteriori informazioni, vedere [Configure power settings](/windows-hardware/customize/power-settings/configure-power-settings).
...
- **Standby connesso.** La modalità predefinita per il recapito di messaggi di posta elettronica, messaggistica e dati sincronizzati sul cloud, lo standby connesso mantiene la Wi-Fi e mantiene la connettività di rete.

- **Standby disconnesso.** Una modalità facoltativa per una durata prolungata della batteria, lo standby disconnesso offre la stessa esperienza istantanea e consente di risparmiare energia disattivando Wi-Fi, Bluetooth e connettività di rete correlata.

Per ulteriori informazioni sullo standby moderno, fare riferimento a [Microsoft Hardware Dev Center](/windows-hardware/design/device-experiences/modern-standby-wake-sources).
...
Surface è progettato per un'esperienza di gestione del risparmio energia semplificata che elimina la necessità di creare combinazioni per il risparmio di energia personalizzate o configurare manualmente le impostazioni di risparmio energia. Microsoft semplifica l'esperienza utente offrendo una singola combinazione per il risparmio di energia (bilanciata) che sostituisce le più combinazioni per il risparmio di energia dalle build Windows standard.Microsoft streamlines the user experience by delivering a single power plan (balanced) that replaces the multiple power plans from standard Windows builds.
...
-   [Personalizzare il dispositivo Windows di alimentazione delle prestazioni](/windows-hardware/customize/desktop/customize-power-slider)

-   [Risparmio batteria.](/windows-hardware/design/component-guidelines/battery-saver)
...
| Configurare la luminosità ottimale dello schermo | Risparmia batteria | Nella barra delle applicazioni seleziona l'icona della batteria e fai clic su **Impostazioni batteria**, seleziona **Luminosità dello schermo inferiore durante il risparmio batteria**. |
...
| Analizzare i problemi relativi alle impostazioni di risparmio energia. | Risoluzione dei problemi di alimentazione | Nella ricerca della barra delle applicazioni per la risoluzione dei problemi seleziona **Risoluzione dei** problemi, quindi seleziona **Alimentazione** e segui le istruzioni.|
...
- [Standby moderno](/windows-hardware/design/device-experiences/modern-standby-wake-sources)
