...
---
# Installare app nel dispositivo Microsoft Surface Hub
...
- Quando invii un'app a Microsoft Store, gli sviluppatori devono impostare le opzioni relative alla disponibilità della famiglia di dispositivi e alle licenze organizzative per verificare che un'app sia disponibile per l'esecuzione su Surface Hub.
...
### Scaricare pacchetti dell'app da Microsoft Store per le aziende
...
**Per distribuire le app in remoto usando Microsoft endpoint Configuration Manager**

> [!NOTE]
> Queste istruzioni si basano sulla filiale corrente di Microsoft endpoint Configuration Manager.

1. Registrare gli hub di Surface in Configuration Manager. Per altre informazioni, vedi [Registrare un dispositivo Surface Hub in MDM](manage-settings-with-mdm-for-surface-hub.md#enroll-into-mdm).
...
12. Distribuisci l'applicazione ai tuoi dispositivi Surface Hub. Per altre informazioni, vedere [distribuire applicazioni con Microsoft endpoint Configuration Manager](https://docs.microsoft.com/sccm/apps/deploy-use/deploy-applications).
13. Se necessario, aggiorna l'app scaricando un nuovo pacchetto dallo Store per le aziende e pubblicando una revisione dell'applicazione in Configuration Manager. Per altre informazioni, vedere [aggiornare e ritirare le applicazioni con Microsoft endpoint Configuration Manager](https://technet.microsoft.com/library/mt595704.aspx). 

> [!NOTE] 
> Se si usa Microsoft endpoint Configuration Manager (Current Branch), è possibile ignorare i passaggi precedenti connettendo lo Store per le aziende a Configuration Manager. In questo modo, puoi sincronizzare l'elenco delle app acquistate con Configuration Manager, visualizzarle nella console di Configuration Manager e distribuirle come si farebbe con qualsiasi altra app. Per altre informazioni, vedere [gestire le app da Microsoft Store per le aziende con Configuration Manager](https://technet.microsoft.com/library/mt740630.aspx).


## Riepilogo

Esistono diversi modi per installare le app nel tuo hub Surface a seconda che tu stia sviluppando app, valuti le app in un numero limitato di dispositivi o Distribuisci le app in generale all'organizzazione. Questa tabella riepiloga i metodi supportati:
